    ...
    for (auto& robot : dataset.robots()) {
        ...
        for(uint64_t i = 0; i < robotMeasurements.size(); ++i){
            ...
            for(uint64_t j = 0; j < entry.measurement_types.size(); ++j){
                if(sampler(generator) && (!outlierTypes.is_initialized() || std::find(outlierTypes->begin(), outlierTypes->end(), entry.measurement_types[j]) != outlierTypes->end())){
                    entry.is_outlier[j] = true;
                    gtsam::NonlinearFactor::shared_ptr outlierFactor = perturbFactor(entry.measurements.at(j), entry.measurement_types[j]);
...
gtsam::NonlinearFactor::shared_ptr perturbFactor(gtsam::NonlinearFactor::shared_ptr factor, std::string tag){
    ...
    Eigen::VectorXd perturb = es.eigenvectors().real().colwise().sum() * std::sqrt(-2*std::log(1 - 0.999) / es.eigenvalues().real().cwiseInverse().squaredNorm());
